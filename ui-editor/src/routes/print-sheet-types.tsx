...
type PrintSheetType = Record<string, unknown>;

export const Route = createFileRoute('/print-sheet-types')({
  component: PrintSheetTypesList,
});

function PrintSheetTypesList() {
  const { data, error, loading } = useApi<PrintSheetType[]>(
  ...
  return (
    <div className="w-full pt-3">
      <div className="mb-3 flex items-end justify-between gap-3">
        <h2 className="text-2xl font-bold">Print Sheet Types</h2>
      </div>
      {loading && !data ? (
        <div className="text-gray-600">Loading print sheet types…</div>
      ) : null}
      ...
      <div className="mb-2 md:hidden">
        <button
          ...
          aria-controls="print-sheet-types-sidebar"
        >
      ...
      <div className="flex gap-4">
        ...
        >
          <SidebarSearch
            value={query}
            onChange={setQuery}
            placeholder="Search print sheet types…"
          />
          {filtered.length === 0 && (
            <div className="p-3 text-xs text-gray-600">
              {debounced
                ? `No results for "${debounced}".`
                : 'No print sheet types found.'}
            </div>
          )}
          {filtered.length > 0 && (
