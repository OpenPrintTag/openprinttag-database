...
export const Route = createFileRoute('/api/devices/accessories')({
  server: {
    middleware: [],
    handlers: {
      GET: async ({ request }) => {
        console.info('GET /api/devices/accessories @', request.url);
        const { readAllEntities } = await import('~/server/data/fs');
        const data = await readAllEntities('devices/accessories', {
